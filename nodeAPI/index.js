...
app.get("/login",function(req,res,next){
    ...
    conx.execute("select id, nom, role, password from users where nom = ?",[user],function(err,data){
        if(err){
            ...
            if(password === data[0].password){
                ...
                res.status(200).send({jwt:token,userName:data[0].nom,userId:data[0].id,role:data[0].role})
            }
            else{
...
app.get("/api/listClubs",auth,(req,res,next)=>{
...
app.post("/api/createMatche",auth,function(req,res){
    const data = req.body;
    console.log("insert into matches (id_guest,id_visitor,date) values('"+Object.values(data).join("','")+"')");
    conx.execute("insert into matches (id_guest,id_visitor,date) values('"+Object.values(data).join("','")+"')",function(err,r){
        if(err){
...
app.delete("/api/deleteMatche/:id",auth,function(req,res){
    ...
    if(role === "admin"){
        ...
        res.status(401).send({msg:"Vous n'êtes pas autorisé à supprimer des matches"})
    }
})
...
app.put("/api/updateScore/:id",auth,function(req,res){
    ...
    conx.execute("update matches set score_guest = "+score.guest+", score_visitor = "+score.visitor+" where id = "+id,function(err,r){
        if(err){
            res.status(500).send({error:err})
        }
