...
async function loadList(x){
    let clubList = []
    await fetch("http://localhost:8080/ige42/arch_rep/phpAPI/listClubs.php")
    .then(function(prm){
        if(prm.status === 500){
            alert(prm.text())
            return false
    ...
    for(let i = 0; i < clubList.length; i++){
        lista += "<li style='width:100%;display:flex;justify-content:space-around;margin:15px'> <img style='width:50px;height:50px' alt="+clubList[i]['nom']+" src='http://localhost:8080/ige42/arch_rep/phpAPI"+clubList[i]['logo']+"' />"+clubList[i]['nom']+"("+clubList[i]['ville']+") <button class='boutton' id='boutton_"+clubList[i]['id']+"'> Supprimer </button></li>"
    }
